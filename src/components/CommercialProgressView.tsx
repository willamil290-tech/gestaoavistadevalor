...
export interface Commercial {
  ...
  currentValue: number;
  goal: number;
  group: "executivo" | "cs" | "closer";
}

export interface CommercialGroup {
  name: string;
  key: "executivo" | "cs" | "closer";
  members: Commercial[];
  totalValue: number;
  totalGoal: number;
}
...
export const CommercialProgressView = ({
  ...
  const [editValues, setEditValues] = useState<{ name: string; currentValue: number; goal: number; group: "executivo" | "cs" | "closer" }>({
    name: "",
    currentValue: 0,
    goal: 0,
    group: "executivo",
  ...
  const startEdit = (c: Commercial) => {
    setEditingId(c.id);
    setEditValues({ name: c.name, currentValue: c.currentValue, goal: c.goal, group: c.group });
  };
  ...
  const groups: CommercialGroup[] = [
    { name: "Executivo", key: "executivo", members: [], totalValue: 0, totalGoal: 0 },
    { name: "CS", key: "cs", members: [], totalValue: 0, totalGoal: 0 },
    { name: "Closer", key: "closer", members: [], totalValue: 0, totalGoal: 0 },
  ];
  ...
  commercials.forEach((c) => {
    ...
    if (group) {
      ...
      group.totalValue += c.currentValue;
      group.totalGoal += c.goal;
    }
  });

  const renderCommercial = (c: Commercial) => {
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setEditValues((v) => ({ ...v, group: e.target.value as "executivo" | "cs" | "closer" }))}
                className="w-full mt-1 p-2 rounded-lg bg-muted border border-border text-sm"
              >
  ...
  const renderGroupSummary = (group: CommercialGroup) => {
    if (group.members.length === 0) return null;
    
    const percentage = group.totalGoal > 0 ? Math.min((group.totalValue / group.totalGoal) * 100, 100) : 0;
    const isGoalReached = group.totalValue >= group.totalGoal;
    ...
    return (
      <div
        ...
        className={cn(
          ...
          isGoalReached ? "bg-green-500/15 border-green-500/50" : "bg-accent/30 border-accent/50"
        )}
      >
        <div className="flex items-center justify-between mb-2">
          <span className={cn("font-bold", tvMode ? "text-xl" : "text-lg")}>
            {group.name} (Consolidado)
          </span>
          <span
            className={cn(
              "font-bold",
        ...
        <div className="flex justify-between mt-2 text-sm text-muted-foreground">
          <span>Total: {fmtBRL(group.totalValue)}</span>
          <span>Meta: {fmtBRL(group.totalGoal)}</span>
        </div>
      </div>
    );
  };
